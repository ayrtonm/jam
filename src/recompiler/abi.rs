...
impl Recompiler {
    ...
    fn free_variables(&mut self) {
        stack!(
            self,
            self.asm.emit_addq_ir(self.alloc.stack().0, X64Reg::RSP)
        );
    }

    fn sysv_callee_prologue(&mut self) {
        let offset = X64Reg::callee_saved_regs()
            .into_iter()
    ...
    fn free_pointers(&mut self) {
        let offset = self
            .asm
            .emit_addq_ir(self.alloc.emulator_ptrs().0, X64Reg::RSP);
        *self.alloc.emulator_ptrs_mut() += offset;
    }
    ...
    fn load_emu_regs(&mut self, inputs: &[EmuRegNameType]) {
        ...
        trash!(self
            .asm
            .emit_addq_ir(-(inputs.len() as StackOffsetType) * 4, X64Reg::RSP));
        let regs = inputs
            .iter()
            .enumerate()
            .map(|(n, &i)| {
        ...
        let regs = regs
            .iter()
            .map(|reg| (reg.name, self.alloc.reg_position(&reg)))
            .collect::<Vec<_>>();
        regs.iter().for_each(|(name, pos)| {
            let src_offset = StackOffset(4 * name.0 as StackOffsetType);
            self.asm
                .emit_movl_mr_offset(X64Reg::RAX, X64Reg::RCX, src_offset);
    ...
    fn save_emu_regs(&mut self) {
        self.asm
            .emit_movq_mr_offset(X64Reg::RSP, X64Reg::RAX, self.alloc.ptr_position(0));
        let regs = self
            .alloc
            .emulator_regs()
            .iter()
            .map(|&r| (r.name, self.alloc.reg_position(&r), r.size))
            .collect::<Vec<_>>();
        let offset = regs
            .into_iter()
            .map(|(name, pos, size)| {
                ...
                self.asm
                    .emit_movl_rm_offset(X64Reg::RCX, X64Reg::RAX, dest_offset);
                size
            })
            .sum::<StackOffset>();
