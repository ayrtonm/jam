...
impl Recompiler {
  ...
  fn set_argn(&mut self, value: JITValue, n: ArgNumber) {
    ...
    bind!(self, match self.alloc.value_to_reg(&value) {
      Some(&value_reg) => {
        self.alloc.swap_specific_bindings(value_reg, arg_reg)
      },
      None => {
