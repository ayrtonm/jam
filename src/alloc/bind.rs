...
impl Allocator {
  ...
  pub fn bind_flags(&mut self) -> MultiTransfer {
    let flag_idx = Idx(self.flag_count());
  ...
  pub fn get_delayed_write(&self, emu_reg: EmuReg) -> Option<&JITValue> {
    self.mappings
        ...
        .filter(|&v| {
          match v {
            JITValue::DelayedWrite(emu_reg,_) => true,
            _ => false,
          }
        })
        .next()
  ...
  pub fn process_delayed_write(&mut self) -> MultiTransfer {
    ...
    if self.write_count() != 0 {
      let idx = Idx(self.write_count() - 1);
      match self.mappings
          .iter()
          .filter(|(_, &v)| {
            match v {
