...
impl Assembler {
    ...
    pub fn emit_movq_mr(&mut self, ptr: X64Reg, dest: X64Reg) {
        let prefix =
            Assembler::REX | Assembler::REXW | Assembler::rexr(dest) | Assembler::rexb(ptr);
        self.emit_u8(prefix);
        self.emit_u8(Assembler::MOV);
        if ptr.low() == 5 {
            self.emit_u8(0x45 | dest.low() << 3);
    ...
    pub fn emit_movq_rm(&mut self, src: X64Reg, ptr: X64Reg) {
        let prefix = Assembler::REX | Assembler::REXW | Assembler::rexr(src) | Assembler::rexb(ptr);
        self.emit_u8(prefix);
        self.emit_u8(Assembler::MOV2);
        if ptr.low() == 5 {
            self.emit_u8(0x45 | src.low() << 3);
    ...
    pub fn emit_movq_rm_offset(&mut self, src: X64Reg, ptr: X64Reg, offset: StackOffset) {
        let offset = offset.0;
        if offset == 0 {
            self.emit_movq_rm(src, ptr);
        } else {
            let prefix =
                Assembler::REX | Assembler::REXW | Assembler::rexr(src) | Assembler::rexb(ptr);
            self.emit_u8(prefix);
            self.emit_u8(Assembler::MOV2);
            match offset {
                -0x80..=0x7f => {
