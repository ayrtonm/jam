...
impl Assembler {
    ...
    pub fn define_label(&mut self, label: Label) {
        self.labels_defined
            .insert(label, StackOffset(self.buffer.len() as StackOffsetType));
    }
    ...
    pub fn emit_transfers(&mut self, transfers: Transfer, stack: StackOffset) {
        ...
        for t in transfers {
            match t.other {
                GenericValue::JITValue(other) => match other {
                    JITValue::Flags(_) => match t.dir {
                        Direction::ToReg => {
                            trash!(self.emit_pushfq());
                            trash!(self.emit_popq_r(t.reg));
                        },
                        Direction::FromReg => {
                            trash!(self.emit_pushq_r(t.reg));
                            trash!(self.emit_popfq());
                        },
                    },
                    _ => {
                        let size = other.size();
